...
export function CartProvider({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{queueMicrotask(()=>{try{const saved=localStorage.getItem("yaqeen-cart");if(saved)setItems(JSON.parse(saved));}catch{}setReady(true)})},[]);
  useEffect(()=>{if(ready)localStorage.setItem("yaqeen-cart",JSON.stringify(items))},[items,ready]);
