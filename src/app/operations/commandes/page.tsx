...
import { confirmDeliveryAction, releaseTransferAction } from "./actions";
...
export default async function OperationsOrdersPage({ searchParams }: { searchParams: Promise<{ confirmed?: string; transferred?: string; error?: string }> }) {
  ...
  return <main className="moderation-shell operations-orders"><header className="moderation-topbar"><Link href="/" className="brand-mark">yaqeen<span>✦</span></Link><b>OPÉRATIONS · COMMANDES</b><div><Link href="/operations/moderation">Modération</Link><Link href="/compte">{viewer.displayName || viewer.email || "Opérateur"}</Link></div></header>
    ...
    {(params.confirmed || params.transferred) && <div className="moderation-flash">✓ Transition enregistrée et auditée.</div>}{params.error && <div className="moderation-flash moderation-flash-error">Opération refusée : l’état, la preuve ou le rapprochement Stripe n’est pas valide.</div>}
