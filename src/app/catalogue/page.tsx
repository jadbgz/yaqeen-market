...
export default async function CataloguePage({
  ...
  return <main className="catalog-shell">
    ...
    <section className="catalog-grid">{products.map((product,index)=><Link href={productHref(product)} className="catalog-card" key={product.id}><div className="catalog-visual catalog-photo"><Image src={product.media[0].url} alt={product.media[0].altText} fill sizes="(max-width: 720px) 100vw, (max-width: 1100px) 50vw, 33vw" unoptimized/><em>Preuve & image revues</em><span>{String(index+1).padStart(2,"0")}</span></div><div className="catalog-info"><p>{product.shop} · Vérifié par Yaqeen</p><div><h2>{product.name}</h2><strong>{formatPrice(product.price,product.currency)}</strong></div></div></Link>)}{products.length===0&&<div className="catalog-empty"><strong>Aucun produit publié pour le moment.</strong><p>Le catalogue se remplit uniquement après revue des boutiques, des preuves et des médias produit.</p>{q||category!=="Tous"?<Link href="/catalogue">Réinitialiser la recherche</Link>:<Link href="/seller">Ouvrir une boutique</Link>}</div>}</section>
