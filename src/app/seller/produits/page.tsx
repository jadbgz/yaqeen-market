...
export default async function SellerProductsPage({ searchParams }: { searchParams: Promise<{ created?: string; submitted?: string; review_error?: string }> }) {
  ...
  return <SellerChrome shopName={dashboard.shop.name} shopStatus={dashboard.shop.status} userName={userName} activeRoute="products"><div className="seller-content seller-products-page">
    ...
    {params.submitted === "1" && <div className="seller-success" role="status"><span>✓</span><div><strong>Produit transmis à la modération.</strong><p>Son contenu et ses images sont verrouillés jusqu’à la décision Yaqeen.</p></div></div>}{params.review_error && <div className="seller-form-feedback">Soumission refusée. Vérifiez la description, la variante, la preuve, au moins une image et le statut de la boutique.</div>}
    {products.length === 0 ? <section className="seller-catalog-empty"><span>◇</span><p>VOTRE PREMIER OBJET MÉTIER</p><h2>Une fiche complète,<br />pas une simple carte.</h2><p>Le produit restera invisible aux clients tant que sa preuve, ses images et son contenu n’auront pas été revus.</p><Link href="/seller/produits/nouveau">Créer le premier brouillon →</Link></section> : <section className="seller-product-list"><header><span>Produit</span><span>Prix</span><span>Stock</span><span>Confiance</span><span>Publication</span><span>Action</span></header>{products.map((product) => <article key={product.id}><div className="seller-product-identity"><span>{product.category.slice(0, 2).toUpperCase()}</span><div><strong>{product.title}</strong><small>{product.variant?.title ?? "Aucune variante"} · {product.variant?.sku ?? "Sans SKU"} · {product.mediaCount}/6 image{product.mediaCount > 1 ? "s" : ""}</small></div></div><strong>{product.variant ? money.format(product.variant.priceCents / 100) : "—"}</strong><span>{product.variant?.availableStock ?? 0}</span><span className={`seller-state seller-state-${product.evidenceStatus ?? "none"}`}>{product.evidenceStatus ? evidenceLabels[product.evidenceStatus] : "Aucune preuve"}</span><span className={`seller-state seller-product-${product.status}`}>{statusLabels[product.status]}</span><div className="seller-product-actions"><Link className="seller-review-button" href={`/seller/produits/${product.id}`}>{product.status === "draft" || product.status === "rejected" ? "Éditer" : "Consulter"}</Link><Link className="seller-review-button" href={`/seller/produits/${product.id}/medias`}>Images</Link>{product.status === "draft" && product.mediaCount > 0 && (dashboard.shop.status === "under_review" || dashboard.shop.status === "approved") ? <form action={submitProductForReview}><input type="hidden" name="productId" value={product.id} /><button className="seller-review-button" type="submit">Soumettre →</button></form> : product.status === "draft" ? <small className="seller-action-hint">Ajoutez une image puis soumettez la boutique</small> : product.status === "rejected" ? <small className="seller-action-hint">Corrigez la fiche avant une nouvelle soumission</small> : <small className="seller-action-hint">Contenu verrouillé</small>}</div></article>)}</section>}
  </div></SellerChrome>;
}
