...
export default async function SellerPage({ searchParams }: { searchParams: Promise<{ shop_submitted?: string; review_error?: string }> }) {
  if (!getSupabaseConfig()) {
    return <SellerGate><section className="seller-public-landing"><div className="seller-public-hero"><p>VENDEZ SUR YAQEEN MARKET</p><h1>Votre boutique.<br /><span>Notre place de marché.</span></h1><p>Préparez un catalogue clair, documentez vos produits et rejoignez une marketplace construite pour rendre la confiance visible — sans effacer votre identité.</p><div><a href="#fonctionnement">Voir le fonctionnement</a><Link href="/catalogue">Explorer la marketplace</Link></div></div><div id="fonctionnement" className="seller-public-steps"><article><span>01</span><h2>Créez votre boutique</h2><p>Un seul compte pour acheter et piloter votre activité.</p></article><article><span>02</span><h2>Documentez vos produits</h2><p>Fiche, stock, médias et preuve sont préparés ensemble.</p></article><article><span>03</span><h2>Passez la revue Yaqeen</h2><p>Rien n’est publié automatiquement ou sans décision tracée.</p></article></div><aside><strong>Ouverture progressive</strong><p>Le Seller Center est activé environnement par environnement avec les premiers vendeurs. Cette page reste utile et compréhensible même lorsqu’aucun compte ne peut encore être créé ici.</p></aside></section></SellerGate>;
  }
